...
public class suicideBomberHandler {
    ...
    public LivingEntity convertToSuicideBomber(final Zombie z) {
        ...
        z.getAttribute(Attribute.GENERIC_MAX_HEALTH)
                .setBaseValue((double) zCM.getSuicideBomberConfig().getDouble("SuicideBomber.MaxHealth"));
        ...
        z.setCustomNameVisible((boolean) zCM.getSuicideBomberConfig().getBoolean("SuicideBomber.isCustomNameVisible"));
        z.getAttribute(Attribute.ZOMBIE_SPAWN_REINFORCEMENTS).setBaseValue((double) 0.03);
        z.getAttribute(Attribute.GENERIC_FOLLOW_RANGE)
                .setBaseValue((double) zCM.getSuicideBomberConfig().getDouble("SuicideBomber.FollowRange"));
        z.getAttribute(Attribute.GENERIC_ATTACK_DAMAGE)
                .setBaseValue((double) zCM.getSuicideBomberConfig().getDouble("SuicideBomber.AttackDamage"));
        z.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED)
                .setBaseValue((double) zCM.getSuicideBomberConfig().getDouble("SuicideBomber.MovementSpeed"));
        z.setRemoveWhenFarAway((boolean) zCM.getSuicideBomberConfig().getBoolean("SuicideBomber.RemoveWhenFarAway"));
        ...
        ItemStack chest = new ItemStack(
                Material.getMaterial(zCM.getSuicideBomberConfig().getString("SuicideBomber.Chestplate")));
        ...
        for (String s : zCM.getSuicideBomberConfig().getStringList("SuicideBomber.PotionEffects")) {
            if (s == null) {
                ...
                z.addPotionEffect(new PotionEffect(PotionEffectType.getByName(split[0]), Integer.parseInt(split[1]),
