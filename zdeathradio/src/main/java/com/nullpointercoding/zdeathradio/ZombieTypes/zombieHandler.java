    ...
    public LivingEntity convertToZombie(final Zombie z) {
        ...
        if (z.getLocation().getChunk().isEntitiesLoaded()) {
            ...
            z.getAttribute(Attribute.MAX_HEALTH)
                    .setBaseValue((double) zCM.getZombieConfig().getDouble("Zombie.MaxHealth"));
            z.getAttribute(Attribute.SPAWN_REINFORCEMENTS).setBaseValue((double) 0.03);
            z.getAttribute(Attribute.FOLLOW_RANGE)
                    .setBaseValue((double) zCM.getZombieConfig().getDouble("Zombie.FollowRange"));
            z.getAttribute(Attribute.ATTACK_DAMAGE)
                    .setBaseValue((double) zCM.getZombieConfig().getDouble("Zombie.AttackDamage"));
            z.getAttribute(Attribute.MOVEMENT_SPEED)
                    .setBaseValue((double) zCM.getZombieConfig().getDouble("Zombie.MovementSpeed"));
            ...
            ItemStack mainHand = new ItemStack(
                    Material.getMaterial(zCM.getZombieConfig().getString("Zombie.MainHand")));
            // ItemStack offHand = new
            // ItemStack(Material.getMaterial(zCM.getZombieConfig().getString("Zombie.OffHand")));
